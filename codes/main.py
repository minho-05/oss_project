...
with st.sidebar:
    ...
    if age_group == "청년층 (20~30대)":
        custom_weights = {
            'subway': 5.0, 'bus': 2.0, 'convenience': 4.0,  'supermarket': 1.0, 'cafe': 4.0,
            'hospital': 1.0, 'public': 1.0, 'bank': 1.0, 'school': 0.0, 'park': 2.0
        }

    elif age_group == "중장년층 (40~50대)":
        custom_weights = {
            'subway': 3.0, 'bus': 2.0, 'convenience': 1.0,  'supermarket': 4.0, 'cafe': 3.0,
            'hospital': 4.0, 'public': 2.0, 'bank': 4.0, 'school': 5.0, 'park': 3.0
        }
        ...
        st.write("가중치 직접 조정")
        custom_weights = {
            'subway': st.slider("지하철 중요도", 0.0, 5.0, 3.0, key='w_subway'),
            'bus': st.slider("버스 중요도", 0.0, 5.0, 3.0, key='w_bus'),
            'convenience': st.slider("편의점 중요도", 0.0, 5.0, 3.0, key='w_conv'),  
            'supermarket': st.slider("마트 중요도", 0.0, 5.0, 3.0, key='w_mart'),
            'cafe': st.slider("카페 중요도", 0.0, 5.0, 3.0, key='w_cafe'),           
            'hospital': st.slider("병원 중요도", 0.0, 5.0, 3.0, key='w_hosp'),
            'public': st.slider("관공서 중요도", 0.0, 5.0, 3.0, key='w_public'),
            'bank': st.slider("은행 중요도", 0.0, 5.0, 3.0, key='w_bank'),
            'school': st.slider("학교 중요도", 0.0, 5.0, 3.0, key='w_school'),      
            'park': st.slider("공원 중요도", 0.0, 5.0, 3.0, key='w_park')
        }
...
if output and output.get('last_clicked'):
    ...
    if (lat, lon) != st.session_state.last_coords:
        ...
        with st.spinner(f"📍 좌표({lat:.4f}, {lon:.4f}) 데이터를 분석 중..."):
    ...
    if data:
        ...
        for label, coords in nearest_locs.items():
            ...
            folium.Marker(
                ...
                popup=f"{label}: {distance_val:.0f}m",
                tooltip=label, 
